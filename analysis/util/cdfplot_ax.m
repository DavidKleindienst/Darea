function [handleCDF,stats] = cdfplot_ax(ax,x)
%Added functionality to choose axis handle
...
hCDF = plot(ax,xCDF , yCDF);
if (nargout>0), handleCDF=hCDF; end
grid  (ax,'on')
xlabel(ax,getString(message('stats:cdfplot:LabelX')))
ylabel(ax,getString(message('stats:cdfplot:LabelFx')))
title (ax,getString(message('stats:cdfplot:Title')))
